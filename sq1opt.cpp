...
// When a concrete piece is declared multiple times, it gets replaced with a tracked wildcard
//  (T for corners, S for edges) that behaves like W/Z but has a distinct value range so the
//  solver can enforce that at least one lands at the piece's solved position.
class FullPosition {
public:
	int pos[24];
	int middle;
...
struct PartialPieceSpec {
	int kind; // 0 concrete, 1 partial corner class, 2 partial edge class
	int value;
	int count;
};

struct PartialPrunKey {
	uint32_t shapeMiddle;
	uint64_t posBits;
	bool operator==(const PartialPrunKey& other) const {
		return shapeMiddle == other.shapeMiddle && posBits == other.posBits;
	}
};

struct PartialPrunKeyHash {
	size_t operator()(const PartialPrunKey& k) const {
		return ((size_t)k.shapeMiddle * 11400714819323198485ull) ^ (size_t)(k.posBits + (k.posBits >> 32));
	}
};

class PartialPrunTable {
	std::vector<PartialPieceSpec> specs;
	std::unordered_map<PartialPrunKey, unsigned char, PartialPrunKeyHash> table;
	ShapeTranTable* stt;
	int maxDepth;
	size_t maxStates;
	bool capped;

	bool specMatches(int piece, const PartialPieceSpec& spec) const {
		if (spec.kind == 0) return piece == spec.value;
		if (spec.kind == 1) return piece < 0 && ((piece % 3 + 3) % 3) == spec.value;
		return piece > 15 && piece % 3 == spec.value;
	}

	bool encode(const FullPosition& p, PartialPrunKey& key) const {
		uint64_t bits = 0;
		int shift = 0;
		int seen[8];
		for(size_t s=0; s<specs.size(); s++) seen[s]=0;
		for (int i=0, pieceIx=0; i<24 && pieceIx<16; pieceIx++) {
			int piece = p.pos[i];
			int code = (piece < 8) ? 0 : 1;
			for (size_t s=0; s<specs.size(); s++) {
				if (specMatches(piece, specs[s])) {
					code = 2 + (int)s;
					seen[s]++;
					break;
				}
			}
			bits |= ((uint64_t)code) << shift;
			shift += 4;
			i += (piece < 8) ? 2 : 1;
		}
		for(size_t s=0; s<specs.size(); s++){
			if(seen[s] != specs[s].count) return false;
		}
		key.shapeMiddle = (p.middle < 0 ? 1u : 0u);
		key.posBits = bits;
		return true;
	}

	int getShapeIndex(FullPosition& p) const {
		int shape = p.getShape();
		int idx = stt->getShape(shape, false);
		if (idx < 0) idx = stt->getShape(shape, true);
		return idx;
	}

	void addTurns(FullPosition start, unsigned char depth, std::queue<FullPosition>& q) {
		std::queue<FullPosition> turnQ;
		std::unordered_map<PartialPrunKey, unsigned char, PartialPrunKeyHash> seenTurns;
		PartialPrunKey key;
		if (!encode(start, key)) return;
		turnQ.push(start);
		seenTurns[key] = 1;
		while (!turnQ.empty()) {
			...
			FullPosition cur = turnQ.front();
			turnQ.pop();
			if (!encode(cur, key)) continue;
			if (table.find(key) == table.end()) {
				table[key] = depth;
				q.push(cur);
			}
			for (int m=0; m<2; m++) {
				FullPosition next = cur;
				if (m==0) next.doTop(1);
				else next.doBot(-1);
				PartialPrunKey nk;
				if (encode(next, nk) && seenTurns.find(nk) == seenTurns.end()) {
					seenTurns[nk] = 1;
					turnQ.push(next);
				}
			}
...
public:
	PartialPrunTable() : stt(NULL), maxDepth(0), maxStates(0), capped(false) {}

	bool build(const std::string& target, const std::vector<PartialPieceSpec>& specs0, ShapeTranTable& stt0, int maxDepth0, size_t maxStates0) {
		specs = specs0;
		stt = &stt0;
		maxDepth = maxDepth0;
		maxStates = maxStates0;
		capped = false;
		table.clear();
		if (specs.empty()) return false;
		FullPosition start;
		int r = start.parseInput(target.c_str());
		if (r) return false;

		std::queue<FullPosition> q;
		addTurns(start, 0, q);
		while (!q.empty()) {
			if (table.size() >= maxStates) {
				capped = true;
			...
			FullPosition cur = q.front();
			q.pop();
			PartialPrunKey key;
			if (!encode(cur, key)) continue;
			unsigned char depth = table[key];
			if (depth >= maxDepth) continue;
			if (!cur.doTwist()) continue;
			addTurns(cur, depth + 1, q);
		}
		return !table.empty() && !capped;
	}

	int lookup(const FullPosition& p) const {
		if (table.empty()) return -1;
		PartialPrunKey key;
		if (!encode(p, key)) return -1;
		std::unordered_map<PartialPrunKey, unsigned char, PartialPrunKeyHash>::const_iterator it = table.find(key);
...
class PositionSolver {
	...
	int search( const int l, const int lm, unsigned long *nodes, int twoGen, bool keepCubeShape){
		...
		if( l==0 ){
			if(isSolved()){
				solMoveLen = moveLen;
				memcpy(solMoveList, moveList, moveLen*sizeof(int));
				if(!batchMode) printsol();
	...
	std::string getSolutionString(){
		...
		int mu=0, md=0;
		if( generator ){
			for( int i=solMoveLen-1; i>=0; i--){
				if( solMoveList[i]==0 ){
					out += printmove(mu, md, (tw==0 && karnotation));
					mu = md = 0;
					out += "/";
					tw++;
				}else if( solMoveList[i]<12 ){
					mu = normaliseMove(mu-solMoveList[i]);
				}else{
					md = normaliseMove(md+solMoveList[i]);
				}
			}
		}else{
			for( int i=0; i<solMoveLen; i++){
				if( solMoveList[i]==0 ){
					out += printmove(mu, md, (tw==0 && karnotation));
					mu = md = 0;
					out += "/";
	...
	int getTwistCount(){
		int tw=0;
		for( int i=0; i<solMoveLen; i++){
			if( solMoveList[i]==0 ) tw++;
		}
		return tw;
	}
	void printsol(){
		std::string out = getSolutionString();
		int tw = getTwistCount();
		int tu=0;
		int mu=0, md=0;
		int angle=0;
		if( generator ){
...
class PartialPositionSolver : public PositionSolver {
	...
	PartialPrunTable* partialPrun;

public:
	PartialPositionSolver( ShapeTranTable& stt0, ShpColTranTable& scte0, ShpColTranTable& sctc0, PrunTable& pr10, PrunTable& pr20 )
	    : PositionSolver(stt0, scte0, sctc0, pr10, pr20), partialPrun(NULL) {}
	void setPartialPrun(PartialPrunTable* partialPrun0) {
		partialPrun = partialPrun0;
	}
	void set(FullPosition& p, bool findAll0, bool ignoreTrans0){
	...
	inline bool prunedOut(int l) {
		if (metric == TWIST_METRIC && partialPrun != NULL) {
			int partialDepth = partialPrun->lookup(fp);
			if (partialDepth > l) return true;
		}
		if (e0>-1 && c0>-1) {
...
std::string createBatchDir(const std::string& baseName){
	std::string dir = baseName;
	if(mkdir(dir.c_str(), 0755)==0) return dir;
	if(errno==EEXIST) return dir;
	for(int n=1; ; n++){
		dir = baseName + "(" + std::to_string(n) + ")";
		if(mkdir(dir.c_str(), 0755)==0) return dir;
		if(errno!=EEXIST){
...
bool moveFile(const std::string& from, const std::string& to){
	...
	in.close();
	out.close();
	if(unlink(from.c_str())!=0) return false;
	return true;
}

void writeCSV(const std::string& path, const std::vector<SolutionResult>& results){
	std::ofstream f(path.c_str());
	if(f.fail()){
		std::cerr<<"Error: cannot write "<<path<<std::endl;
		return;
	}
	f<<"input,solution,slices"<<std::endl;
	for(size_t i=0; i<results.size(); i++){
		const SolutionResult& r = results[i];
		f<<r.input<<","<<stripCommas(r.solution)<<","<<r.slices<<std::endl;
	}
	f.close();
	std::cout<<"Wrote "<<path<<" ("<<results.size()<<" entries)"<<std::endl;
}

void writeStats(const std::string& path, const std::vector<SolutionResult>& results, int errorCount){
	std::ofstream f(path.c_str());
	if(f.fail()){
		std::cerr<<"Error: cannot write "<<path<<std::endl;
		return;
	...
	int maxSlices=0;
	for(size_t i=0; i<results.size(); i++){
		if(results[i].solved && results[i].slices>maxSlices) maxSlices=results[i].slices;
	}
	for(int s=0; s<=maxSlices; s++){
		int count=0;
		for(size_t i=0; i<results.size(); i++){
			if(results[i].solved && results[i].slices==s) count++;
		}
		f<<s<<" slice solutions: "<<count<<std::endl;
	}
	double totalSlices=0;
	int solvedCount=0;
	for(size_t i=0; i<results.size(); i++){
		if(results[i].solved){
			totalSlices += results[i].slices;
			solvedCount++;
		}
	}
	if(solvedCount>0){
		char buf[64];
		snprintf(buf, sizeof(buf), "%.2f", totalSlices/solvedCount);
		f<<"average slice count: "<<buf<<std::endl;
	}
	f<<"errors: "<<errorCount<<std::endl;
	f.close();
	std::cout<<"Wrote "<<path<<std::endl;
}

void countDeclaredPieces(const std::string& line, int concrete[16], int partial[6]){
	for(int i=0; i<16; i++) concrete[i]=0;
	for(int i=0; i<6; i++) partial[i]=0;
	if(line.size() < 16) return;
	for(int i=0; i<16; i++){
		char c=line[i];
		if(c>='a' && c<='z') c += ('A'-'a');
		if(c>='A' && c<='H') concrete[c-'A']++;
		else if(c>='1' && c<='8') concrete[c-'1'+8]++;
		else if(c>='U' && c<='Z') partial[c-'U']++;
	}
}

bool parseTemplatePrun(const std::string& tpl, std::string& target, std::vector<PartialPieceSpec>& specs){
	std::string clean;
	...
	if(clean.size() != 16 && clean.size() != 17) return false;
	int counts[6] = {0,0,0,0,0,0};
	target = clean.substr(0, 16);
	specs.clear();
	for(int i=0; i<16; i++){
		char c = target[i];
		if(c>='a' && c<='z') c += ('A'-'a');
...
bool inferPartialPrun(const std::string& path, std::string& target, std::vector<PartialPieceSpec>& specs){
	std::ifstream f(path.c_str());
	if(f.fail()) return false;
	int minConcrete[16], minPartial[6];
	for(int i=0; i<16; i++) minConcrete[i]=1000000;
	for(int i=0; i<6; i++) minPartial[i]=1000000;
	int usefulLines=0;
	std::string line;
	while(std::getline(f, line)){
		...
		if(empty || line.size() < 16) continue;
		int concrete[16], partial[6];
		countDeclaredPieces(line, concrete, partial);
		for(int i=0; i<16; i++) if(concrete[i] < minConcrete[i]) minConcrete[i] = concrete[i];
		for(int i=0; i<6; i++) if(partial[i] < minPartial[i]) minPartial[i] = partial[i];
		usefulLines++;
	}
	if(usefulLines == 0) return false;

	bool selectedConcrete[16];
	for(int i=0; i<16; i++) selectedConcrete[i]=false;
	int tracked=0;
	for(int i=0; i<16 && tracked<8; i++){
		if(minConcrete[i] > 0){
			selectedConcrete[i] = true;
			PartialPieceSpec spec;
			spec.kind = 0;
			spec.value = i;
			spec.count = 1;
	...
	const int partialOrder[4] = {1, 0, 3, 4}; // V,U,X,Y; W/Z are reserved as untracked fillers
	for(int oi=0; oi<4 && tracked<8; oi++){
		int p = partialOrder[oi];
		int add = minPartial[p];
		while(add > 0 && tracked < 8){
			PartialPieceSpec spec;
			spec.kind = (p < 3) ? 1 : 2;
			spec.value = (p < 3) ? p : p-3;
			spec.count = 1;
			if(!specs.empty() && specs.back().kind == spec.kind && specs.back().value == spec.value){
				specs.back().count++;
			}else{
				specs.push_back(spec);
			}
	...
	const char solved[17] = "A1B2C3D45E6F7G8H";
	target.assign(solved, solved+16);
	for(int i=0; i<16; i++){
		int piece = -1;
		char c = target[i];
		if(c>='A' && c<='H') piece = c-'A';
		else piece = c-'1'+8;
		if(!selectedConcrete[piece]) target[i] = (piece < 8) ? 'W' : 'Z';
	}
	for(size_t s=0; s<specs.size(); s++){
		if(specs[s].kind == 0) continue;
		int remaining = specs[s].count;
		char ch;
		if(specs[s].kind == 1) ch = "UVW"[specs[s].value];
		else ch = "XYZ"[specs[s].value];
		for(int i=0; i<16 && remaining>0; i++){
			char solvedChar = solved[i];
			int piece = (solvedChar>='A' && solvedChar<='H') ? solvedChar-'A' : solvedChar-'1'+8;
			if(selectedConcrete[piece]) continue;
			if(specs[s].kind == 1 && piece >= 8) continue;
			if(specs[s].kind == 2 && piece < 8) continue;
			if(specs[s].value == 0 && piece >= 4 && piece < 8) continue;
			if(specs[s].value == 1 && piece < 4) continue;
			if(specs[s].kind == 2 && specs[s].value == 0 && piece >= 12) continue;
			if(specs[s].kind == 2 && specs[s].value == 1 && piece < 12) continue;
			target[i] = ch;
			remaining--;
		}
	}
...
int main(int argc, char* argv[]){
	...
	for( int i=1; i<argc; i++){
		if( argv[i][0]=='-' ){
			switch( argv[i][1] ){
				case 'w':
				case 'W':
					metric=TWIST_METRIC; metricExplicit=true; break;
				case 'l':
				case 'L':
					metric=ANGLE_METRIC; metricExplicit=true; break;
				case 'x':
			...
			// check if this is a readable file (batch mode)
	...
	if (batchFile != NULL && !metricExplicit) metric = TWIST_METRIC;
	...
	}else if( batchFile==NULL && numpos<0 ){
		help();
		return 0;
	}
	...
	PartialPrunTable dynamicPartialPrun;
	if(batchFile != NULL && metric == TWIST_METRIC){
		std::string partialTarget;
		std::vector<PartialPieceSpec> partialSpecs;
		if(inferPartialPrun(batchFile, partialTarget, partialSpecs)){
			if(verbosity>=3) std::cout<<"Building dynamic partial pruning table from "<<partialTarget<<std::endl;
			if(dynamicPartialPrun.build(partialTarget, partialSpecs, st, 5, 50000)){
				if(verbosity>=2) std::cout<<"Dynamic partial pruning states: "<<dynamicPartialPrun.size()<<std::endl;
				if(dynamicPartialPrun.size() >= 1000){
					pps.setPartialPrun(&dynamicPartialPrun);
				}else if(verbosity>=2){
					std::cout<<"Dynamic partial pruning table too small; using existing partial search"<<std::endl;
				}
			}
		}
	}
	...
	// batch mode: positional arg was a file
	if(batchFile!=NULL){
		batchMode = true;
		verbosity = 0;
		std::string batchPath(batchFile);
		std::string baseName = getBaseName(batchPath);
		std::string dir = createBatchDir(baseName);
		std::string newPath = dir + "/" + baseName + ".txt";

		// handle name collision for the file inside the directory
		{
			std::ifstream test(newPath.c_str());
			if(test.good()){
				test.close();
				// file already exists in dir, use a different name
				for(int n=1; ; n++){
					newPath = dir + "/" + baseName + "(" + std::to_string(n) + ").txt";
					std::ifstream test2(newPath.c_str());
					if(!test2.good()) break;
					test2.close();
				}
			}
		...
		std::ifstream batchIn(batchPath.c_str());
		if(batchIn.fail()){
			std::cerr<<"Error: cannot open "<<batchPath<<std::endl;
			return 1;
		}
		std::string csvPath = dir + "/" + baseName + ".csv";
		std::ofstream csv(csvPath.c_str());
		if(csv.fail()){
			std::cerr<<"Error: cannot write "<<csvPath<<std::endl;
			return 1;
		}
		csv<<"input,solution,slices"<<std::endl;

		int errorCount=0;
		int solvedCount=0;
		int totalCount=0;
		double totalSlices=0;
		std::vector<int> sliceCounts;
		char buffer[2000];
		int lineNum=0;

		while(batchIn.good()){
			batchIn.getline(buffer,1999);
			if(!batchIn.good() && strlen(buffer)==0) break;
			lineNum++;
			...
			if(r){
				std::cerr<<"Line "<<lineNum<<": parse error ("<<errors[r-1]<<"), skipping"<<std::endl;
				csv<<line<<",,0"<<std::endl;
				errorCount++;
				totalCount++;
				continue;
			...
			if(p.isPartial()){
				...
				int sr = pps.solve(twoGen, extraMoves, keepCubeShape);
				if(sr){
					std::cerr<<"Line "<<lineNum<<": unsolvable ("<<errors[sr-1]<<")"<<std::endl;
					csv<<line<<",,0"<<std::endl;
					errorCount++;
					totalCount++;
					continue;
				}
				solStr = pps.getSolutionString();
				slices = pps.getTwistCount();
			}else{
				ps.set(p, findAll, ignoreTrans);
				int sr = ps.solve(twoGen, extraMoves, keepCubeShape);
				if(sr){
					std::cerr<<"Line "<<lineNum<<": unsolvable ("<<errors[sr-1]<<")"<<std::endl;
					csv<<line<<",,0"<<std::endl;
					errorCount++;
				...
				solStr = ps.getSolutionString();
				slices = ps.getTwistCount();
			}

			totalCount++;
			solvedCount++;
			totalSlices += slices;
			if((int)sliceCounts.size() <= slices) sliceCounts.resize(slices+1, 0);
			sliceCounts[slices]++;
			csv<<line<<","<<stripCommas(solStr)<<","<<slices<<std::endl;
			std::cout<<"["<<totalCount<<"] "<<line<<" -> "<<solStr<<" ["<<slices<<" slices]"<<std::endl;
		}

		batchIn.close();
		csv.close();
		std::cout<<"Wrote "<<csvPath<<" ("<<totalCount<<" entries)"<<std::endl;

		std::string statsPath = dir + "/stat.txt";
		std::ofstream stats(statsPath.c_str());
		if(stats.fail()){
			std::cerr<<"Error: cannot write "<<statsPath<<std::endl;
		}else{
			for(size_t s=0; s<sliceCounts.size(); s++){
				stats<<s<<" slice solutions: "<<sliceCounts[s]<<std::endl;
			}
			if(solvedCount>0){
				char buf[64];
				snprintf(buf, sizeof(buf), "%.2f", totalSlices/solvedCount);
				stats<<"average slice count: "<<buf<<std::endl;
			}
			stats<<"errors: "<<errorCount<<std::endl;
			stats.close();
			std::cout<<"Wrote "<<statsPath<<std::endl;
		}

		if(!moveFile(batchPath, newPath)){
			std::cerr<<"Error: cannot move "<<batchPath<<" to "<<newPath<<std::endl;
			return 1;
		}
		std::cout<<"Moved "<<batchPath<<" -> "<<newPath<<std::endl;
		std::cout<<"Done: "<<solvedCount<<" solved, "<<errorCount<<" errors, "<<totalCount<<" total"<<std::endl;
		return 0;
	}
